...
public:
	...
	*/
	...
	/*!
	 * \brief Size in bytes of the internal hierarchical-Z (ZTile) buffer for the
	 *        current resolution. Layout depends only on resolution + SIMD tier, so
	 *        two instances with matching SetResolution() + Implementation share an
	 *        identical layout and the raw bytes can be transferred between them.
	 */
	virtual size_t GetHiZBufferSize() const = 0;

	/*!
	 * \brief Copy the raw ZTile buffer OUT to dst (>= GetHiZBufferSize() bytes).
	 *        Lets an external process snapshot a built mask for transfer.
	 */
	virtual void GetHiZBuffer(void *dst) const = 0;

	/*!
	 * \brief Overwrite the raw ZTile buffer from src (>= GetHiZBufferSize() bytes).
	 *        The source MUST come from an instance with identical resolution + SIMD
	 *        tier. After this the instance answers TestRect() against the copied
	 *        mask without re-rasterizing — used to run occlusion queries in a
	 *        process that didn't build the mask.
	 */
	virtual void SetHiZBuffer(const void *src) = 0;
