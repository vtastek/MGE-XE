...
public:
	...
	size_t GetHiZBufferSize() const override
	{
		return (size_t)sizeof(ZTile) * mTilesWidth * mTilesHeight;
	}

	void GetHiZBuffer(void *dst) const override
	{
		if (mMaskedHiZBuffer != nullptr && dst != nullptr)
			memcpy(dst, mMaskedHiZBuffer, GetHiZBufferSize());
	}

	void SetHiZBuffer(const void *src) override
	{
		if (mMaskedHiZBuffer != nullptr && src != nullptr)
			memcpy(mMaskedHiZBuffer, src, GetHiZBufferSize());
	}
