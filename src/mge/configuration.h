...
struct ConfigurationStruct {
    ...
    bool ForgeEngineCullTakeover;   // MSOC retirement D2: install MGE's OWN NiAVObject::CullShow detour (enginecull.cpp) instead of depending on msoc.dll for the engine-driven discovery feed. Refuses to install if that prologue is already detoured OR msoc.dll is loaded — two detours on one 5-byte patch is unrecoverable, so the msoc lua mod is the A/B (present = msoc's path, absent + this on = ours). D2 runs NON-DEFERRING: the traversal is engine-faithful but produces no feed, so buildFrustumVisibleSet takes its frustum-only fallback — that isolates "is our CullShow correct?" from "is our feed correct?". Startup-only (no trampoline, cannot be unpatched). Default off.
