...
struct ConfigurationStruct {
    ...
    bool UseRenderProcess;          // master switch for the Forge takeover: bring up the out-of-process 64-bit renderer (mgeHost64) and composite its output into MW's window. Gates RenderInit + framebuffer-vec setup; per-frame blit additionally toggled by a debug key (F11). Default ON — the only knob left that turns the render takeover off wholesale (no-DX12 machines, seam bring-up debugging). Everything the takeover does downstream of this is unconditional; the bring-up A/B flags they used to ride on are gone (see 2026-07-26 "make all defaults").
